...
public class SplitTerrain : EditorWindow {
	...
	void SplitIt()
	{
		...
		{										
			...
			Terrain genTer = tgo.GetComponent<Terrain>();
			...
			// 	Vector3 tiPos = ti.position;			
												
			// 	// First section	
			// 	if ( i==0 && tiPos.x > 0f && tiPos.x < 0.5f && tiPos.z > 0f && tiPos.z < 0.5f )
			// 	{
			// 		// Recalculate new tree position	
			// 		ti.position = new Vector3( tiPos.x * 2f, tiPos.y, tiPos.z * 2f );		
			// 		// Add tree instance						
			// 		genTer.AddTreeInstance( ti );												
			// 	}	
			// 	// Second section
			// 	if ( i==1 && tiPos.x > 0.0f && tiPos.x < 0.5f && tiPos.z >= 0.5f &&	tiPos.z <= 1.0f )
			// 	{
			// 		ti.position = new Vector3( (tiPos.x ) * 2f, tiPos.y, ( tiPos.z - 0.5f ) * 2f );
			// 		genTer.AddTreeInstance( ti );												
			// 	}	
			// 	// Third section
			// 	if ( i==2 && tiPos.x >= 0.5f && tiPos.x <= 1.0f && tiPos.z > 0.0f && tiPos.z < 0.5f )
			// 	{
			// 		ti.position = new Vector3( (tiPos.x - 0.5f ) * 2f, tiPos.y, ( tiPos.z ) * 2f );
			// 		genTer.AddTreeInstance( ti );												
			// 	}					
			// 	// Fourth section
			// 	if ( i==3 && tiPos.x >= 0.5f && tiPos.x <= 1.0f && tiPos.z >= 0.5f && tiPos.z <= 1.0f )
			// 	{
			// 		ti.position = new Vector3( (tiPos.x - 0.5f ) * 2f, tiPos.y, ( tiPos.z - 0.5f ) * 2f );				
			// 		genTer.AddTreeInstance( ti );												
			// 	}
			// }											
			#endregion	
