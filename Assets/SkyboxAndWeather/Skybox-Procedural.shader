...
Properties {
    ...
    [Toggle(SUNDISK_HQ)]
    _ToggleSunHQ ("HQ Sun", Float) = 0

    // [KeywordEnum(None, Simple, High Quality)] _SunDisk ("Sun", Int) = 1
...
SubShader {
    ...
    Pass {
        ...
        #pragma shader_feature SUNDISK_HQ
        ...
        #if SUNDISK_HQ//SKYBOX_SUNDISK == SKYBOX_SUNDISK_HQ
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
        ...
        half lightColorIntensity = clamp(length(_SunColor), 0.25, 1);
        
        #if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
            OUT.sunColor    = kSimpleSundiskIntensityFactor * saturate(cOut * kSunScale) * _SunColor / lightColorIntensity;
        #else // SKYBOX_SUNDISK_HQ
            OUT.sunColor    = kHDSundiskIntensityFactor * saturate(cOut) * _SunColor / lightColorIntensity;
        #endif
        ...
        {
        #if SUNDISK_HQ//SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
